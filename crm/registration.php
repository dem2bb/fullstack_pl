...
$data['phone'] = isset($_POST['custom_phone']) ? str_replace(['+', '(', ')',' ', '-'], '', $_POST['custom_phone']) : '';
...
$data['utmcsr']       = isset($_POST['utmcsr']) ? $_POST['utmcsr'] : '';
$data['utmccn']       = isset($_POST['utmccn']) ? $_POST['utmccn'] : '';

$fullName = explode(' ', $data['name'], 2);
$data['first_name'] = $fullName[0];
$data['last_name'] = empty($fullName[1]) ? '-' : $fullName[1];
$data['product_name'] = 'FSOn1_PL';
...
if (!isset($contact['data'][0]['id'])) {
    $jsondata = array(
        ...
        "utmcsr"       => (string)$data['utmcsr'],
        "utmccn"       => (string)$data['utmccn']
    );
